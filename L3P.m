...
% st: allow empty or a 1x3 numeric row vector [B, C, D]
addOptional(p, 'st', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 3 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));

% L: allow empty or a 1x3 numeric row vector of lower bounds
addOptional(p, 'L', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 3 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));

% U: allow empty or a 1x3 numeric row vector of upper bounds
addOptional(p, 'U', [], @(v) ...
    (isempty(v) || ...
    (isnumeric(v) && isreal(v) && isrow(v) && numel(v) == 3 && ...
     all(isfinite(v(:))) && ~any(isnan(v(:))))));
